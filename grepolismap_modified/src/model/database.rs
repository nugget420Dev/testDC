...
use crate::town::Town;
use crate::island::Island as ViewIsland;
...
impl From<&Island> for ViewIsland {
    fn from(value: &Island) -> Self {
        Self {
            id: value.id,
            x: value.x,
            y: value.y,
...
impl DataTable {
    ...
    pub fn get_all_islands(&self) -> Vec<ViewIsland> {
        self.islands.iter().map(|i| i.deref().into()).collect()
    }
