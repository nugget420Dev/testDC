        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::Frame::canvas(ui.style()).show(ui, |ui| {
                ...
                let visible_ghost_towns: Vec<&Town> = self
                    ...
                    .filter(|town| filter.town_in_viewport(town))
                    .collect();
                let visible_islands: Vec<&crate::island::Island> = self
                    .ui_data
                    .all_islands
                    .iter()
                    .filter(|island| {
                        let island_x = f32::from(island.x);
                        let island_y = f32::from(island.y);
                        filter.x_in_viewport(island_x) && filter.y_in_viewport(island_y)
                    })
                    .collect();

                // DRAW WATER BACKGROUND
                // Fill the entire viewport with water color
                painter.rect_filled(
                    response.rect,
                    0.0,
                    egui::Color32::from_rgb(10, 60, 110), // Dark blue water
                );

                // DRAW ISLANDS
                for island in &visible_islands {
                    let screen_pos = canvas_data
                        .world_to_screen(egui::vec2(f32::from(island.x), f32::from(island.y)))
                        .to_pos2();
                    
                    // Determine island size and color based on type
                    // island_id: 1-10 = player islands with farm towns
                    // island_id: 11-16 = uninhabited islands
                    // island_id: 17-21 = rocks
                    let (island_size, island_color) = match island.typ {
                        1..=10 => {
                            // Player islands - larger, greenish-brown
                            let size = 1.2 + (f32::from(island.towns) * 0.15);
                            (size, egui::Color32::from_rgb(160, 140, 90))
                        }
                        11..=16 => {
                            // Uninhabited islands - medium, gray-green
                            (0.8, egui::Color32::from_rgb(120, 130, 100))
                        }
                        17..=21 => {
                            // Rocks - small, dark gray
                            (0.5, egui::Color32::from_rgb(80, 80, 80))
                        }
                        _ => {
                            // Unknown type - magenta for debugging
                            (0.7, egui::Color32::from_rgb(255, 0, 255))
                        }
                    };

                    // Draw island as a filled circle
                    painter.circle_filled(
                        screen_pos,
                        canvas_data.scale_world_to_screen(island_size),
                        island_color,
                    );

                    // Add a darker outline for islands at higher zoom levels
                    if canvas_data.zoom > 3.0 {
                        painter.circle_stroke(
                            screen_pos,
                            canvas_data.scale_world_to_screen(island_size),
                            egui::Stroke::new(1.0, island_color.linear_multiply(0.7)),
                        );
                    }
                }
